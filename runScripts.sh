...
if [ $? -eq 0 ]; then
    ...
    # los acuerdos requieren de promociones ingresadas
    run_test --spec cypress/e2e/Funcionario/009_AgregarDocumentoPromocion.cy.js 
    if [ $? -eq 0 ]; then
        run_test--spec cypress/e2e/Funcionario/027_AgregarAcuerdo-PublicarBoletin.cy.js 
    else
        echo "Hubo un error en la carga de la promocion en la prueba 009 de funcionario"
    fi

    run_test --spec cypress/e2e/Funcionario/028_Reportes.cy.js 
    run_test --spec cypress/e2e/Funcionario/029_PanelFimasPendientes.cy.js 
    run_test --spec cypress/e2e/Funcionario/014_TurnarExpediente-Interno.cy.js 

    if [ $? -eq 0 ]; then
        run_test --spec cypress/e2e/Funcionario/501_RecibirExpedienteTurnado.cy.js 
        run_test --spec cypress/e2e/Funcionario/007_ActionButtonsEnabled.cy.js 
        run_test --spec cypress/e2e/Funcionario/008_AcuerdosNotificacionesSentenciasOficios.cy.js 
        run_test --spec cypress/e2e/Funcionario/009_AgregarDocumentoPromocion.cy.js 
        run_test --spec cypress/e2e/Funcionario/010_HabilitaVistaDocumentosACiudadano.cy.js 
        run_test --spec cypress/e2e/Funcionario/023_TurnarExpediente-Externo.cy.js 
    else
